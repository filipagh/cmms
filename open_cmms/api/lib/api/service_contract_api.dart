...
class ServiceContractApi {
  ...
  Future<Response> getServiceContractContractsForComponentGetWithHttpInfo(
    String componentId,
  ) async {
    // ignore: prefer_const_declarations
    final path = r'/service-contract/contracts_for_component';
  ...
  ///
  /// * [String] componentId (required):
  Future<List<ServiceContractSchema>?>
      getServiceContractContractsForComponentGet(
    String componentId,
  ) async {
    final response =
        await getServiceContractContractsForComponentGetWithHttpInfo(
      componentId,
    );
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
