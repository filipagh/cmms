...
class AssetSchema {
  /// Returns a new [AssetSchema] instance.
  AssetSchema({
  ...
  static AssetSchema? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "AssetSchema[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "AssetSchema[$key]" has a null value in JSON.');
        });
        return true;
      }());
  ...
  static List<AssetSchema>? listFromJson(dynamic json, {bool growable = false,}) {
    ...
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = AssetSchema.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, AssetSchema> mapFromJson(dynamic json) {
    final map = <String, AssetSchema>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = AssetSchema.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of AssetSchema-objects as value to a dart map
